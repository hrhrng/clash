...
const ContextNode = ({ data, selected, id }: NodeProps) => {
    const [showModal, setShowModal] = useState(false);
    const [label, setLabel] = useState(data.label || 'Context');
    const [content, setContent] = useState(data.content || '# Context\nAdd background information here...');
    ...
    const handleCancel = useCallback(() => {
        ...
        setLabel(data.label || 'Context');
        setContent(data.content || '# Context\nAdd background information here...');
    }, [data.label, data.content]);
    ...
    return (
        <>
            ...
            >
                ...
                >
                    <input
                        className="bg-transparent text-lg font-bold text-amber-600 focus:text-amber-700 focus:outline-none"
                        value={label}
                        onChange={handleLabelChange}
                        placeholder="Context"
                    />
                ...
                {/* Main Card - No handle, for background context */}
                <div className={`w-full h-full bg-[#FFFBF0] rounded-matrix flex flex-col overflow-hidden transition-all duration-300 hover:shadow-xl ${
                    selected ? 'ring-4 ring-amber-500 ring-offset-2' : 'ring-1 ring-amber-200'
                }`}>
                    {/* Card Content */}
                    <div className="flex-1 p-8 flex flex-col relative">
                        ...
                        <div className="flex-1 relative overflow-hidden">
                            ...
                            <div className="absolute bottom-0 left-0 right-0 h-32 bg-gradient-to-t from-[#FFFBF0] to-transparent pointer-events-none" />
                        </div>
                    </div>
                </div>

                {/* No handles - this is context/background information only */}
            </div>
...
const MarkdownPreview = ({ content }: { content: string }) => {
    return (
        <div
            className="prose prose-lg max-w-none prose-slate prose-headings:font-bold prose-headings:text-slate-900 prose-p:text-slate-700 prose-a:text-amber-600 prose-code:text-amber-600 prose-code:bg-amber-50 prose-code:px-1.5 prose-code:py-0.5 prose-code:rounded"
