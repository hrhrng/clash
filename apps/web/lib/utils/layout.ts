 ...
 */
export function calculateGroupBounds(groupId: string, nodes: Node[]): { width: number; height: number } {
 ...
 */
export function expandGroupToFit(group: Node, childAbsoluteRect: Rect, nodes: Node[]): { width: number; height: number } {
...
export function autoPlaceNode(
    ...
    const finalPos = findNonOverlappingPosition(targetPos, newNodeSize.width, newNodeSize.height, nodes, nodes, parentGroupId);
