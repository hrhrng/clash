...
async def create_session(thread_id: str, project_id: str, title: str | None = None) -> None:
    ...
    try:
        ...
        db.commit()
        logger.info(f"[Session] Created/updated session: thread_id={thread_id}, project_id={project_id}, title={title}")
    finally:
        db.close()
...
async def request_interrupt(thread_id: str) -> bool:
    ...
    try:
        ...
        rows = db.fetchall("SELECT thread_id FROM session_interrupts WHERE thread_id = ? AND status = 'completing'", (thread_id,))
        success = len(rows) > 0
        
        if success:
            logger.info(f"[Session] Interrupt requested: thread_id={thread_id}")
        else:
            logger.warning(f"[Session] Interrupt failed - session not found or not running: thread_id={thread_id}")
...
def check_interrupt_flag(thread_id: str) -> bool:
    ...
    try:
        ...
        status = rows[0][0] if isinstance(rows[0], (list, tuple)) else rows[0].get("status")
        should_interrupt = status in ("completing", "interrupted")
        
        if should_interrupt:
            logger.debug(f"[Session] Interrupt flag checked - TRUE: thread_id={thread_id}, status={status}")
...
def get_session_status(thread_id: str) -> SessionStatus | None:
    ...
    try:
        ...
        if not rows:
            return None
        return rows[0][0] if isinstance(rows[0], (list, tuple)) else rows[0].get("status")
    finally:
        db.close()
...
def get_session_history(thread_id: str) -> list[dict[str, Any]]:
    ...
    def generate_id():
        ...
        import string
        import time
        return str(int(time.time() * 1000)) + "".join(random.choices(string.ascii_lowercase + string.digits, k=7))
    ...
    logger.info(f"[SessionHistory] Generated {len(history)} display items for thread_id={thread_id}")
    return history
        ...
        for row in rows:
            if isinstance(row, (list, tuple)):
                ...
                history.append({
                    ...
                    "title": row.get("title") or f"Session {row.get('thread_id')[-6:]}",
...
class SessionEventEncoder(json.JSONEncoder):
    """Custom JSON encoder for session events to handle LangChain messages."""
    def default(self, obj: Any) -> Any:
        # Handle LangChain messages and other objects with .dict() or to_json()
...
def get_session_history_from_events(thread_id: str) -> list[dict[str, Any]]:
    ...
    for event in events:
        ...
        if etype == "user_message":
            ...
            if last_item and last_item["type"] == "message" and last_item.get("role") == "assistant" and last_item.get("agent_id") == agent_id:
            ...
            last_item = display_items[-1] if display_items else None
            if last_item and last_item["type"] == "thinking" and last_item.get("agent_id") == agent_id:
                last_item["content"] += content
            else:
                display_items.append({
            ...
            display_items.append({
                ...
                "props": {
                    "agentId": data.get("id") if data.get("tool") == "task_delegation" else None,
                    "agentName": data.get("input", {}).get("agent", "Specialist") if data.get("tool") == "task_delegation" else None,
